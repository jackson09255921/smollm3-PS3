...
class MyPS3VLMModel(Idefics3Model):
    """
    這就是我們的客製化模型核心：
    1. 繼承 Idefics3Model (為了 text_model 和基礎框架)。
    2. 在 __init__ 中，覆寫 self.vision_model 和 self.connector。
    3. 複製 SmolVLMModel (Code File 2) 的 forward 和 inputs_merger 邏輯。
    """
    
    def __init__(self, config):
        # 1. 呼叫父類 __init__
        # 這會載入 Idefics3 的所有原始組件，包括 text_model
        super().__init__(config)
        
        # ----------------------------------------------------
        # 2. !! 覆寫 Vision Encoder (載入 PS3) !!
        # ----------------------------------------------------
        self.ps3_model_id = "nvidia/PS3-4K-SigLIP2"
        # 允許從 config 覆寫 'num_look_close'，預設為 2 (如範例所示)
        self.ps3_num_look_close = getattr(config, "ps3_num_look_close", 2)
        
        logger.info(f"正在從 {self.ps3_model_id} 載入 PS3VisionModel...")
        
        # 丟棄 super() 載入的 self.vision_model，換成 PS3
        ...
        # 3. !! 覆寫 Projector (連接器) !!
        # ----------------------------------------------------
        # 從 PS3 範例中我們知道 D_ps3 是 1152
        D_ps3 = self.vision_model.config.hidden_size # (應為 1152)
        
        # 從 Idefics3 config 獲取 LLM 的維度
        D_llm = config.text_config.hidden_size # (例如 4096)
        
        # 建立一個新的、簡單的線性層作為 Projector
        self.connector = nn.Linear(D_ps3, D_llm, bias=False)

        logger.info(f"MyPS3VLMModel 初始化完畢：")
        logger.info(f"  Vision Encoder: PS3 (輸出維度: {D_ps3})")
        logger.info(f"  Projector: nn.Linear({D_ps3}, {D_llm})")
        logger.info(f"  LLM: {self.text_model.__class__.__name__} (輸入維度: {D_llm})")
        logger.info(f"  PS3 num_look_close 參數設為: {self.ps3_num_look_close}")

    # ------------------------------------------------------------------------
    # 4. 完整複製 SmolVLMModel (Code File 2) 的 `inputs_merger`
    #    (這段程式碼 100% 可重用，它很完美)
    # ------------------------------------------------------------------------
    def inputs_merger(
        self,
    ...
    @add_start_docstrings_to_model_forward(
        ...
        # """,
        """
        Forward pass for MyPS3VLMModel.
        """,
    ...
    def forward(
        ...
        pixel_attention_mask: Optional[torch.BoolTensor] = None, # PS3 不會用到
        ...
        # --- (複製 SmolVLMModel 的所有 'forward' 初始設定程式碼) ---
        ...
        # ... (複製 gradient checkpointing, input_ids/inputs_embeds 檢查, past_key_values 邏輯) ...
        ...
        if pixel_values is not None and image_hidden_states is not None:
            ...
            # --- (複製 SmolVLMModel 的 pixel_values 處理邏輯) ---
            ...
            # (我們不再需要 pixel_attention_mask 或 patch_attention_mask 相關邏輯)
            ...
            # ================================================================
            # !! 核心修改 !!
            # 呼叫 PS3 vision model，使用它自己的 forward 參數
            # ================================================================
        ...
        # 呼叫我們的 inputs_merger (torch.cat 版本)
...
class MyPS3VLMForConditionalGeneration(Idefics3ForConditionalGeneration):
    """
    這就是你最終要載入的「外殼」類別。
    它會將 self.model 指向我們上面定義的 MyPS3VLMModel。
    """
    ...
    def __init__(self, config):
        ...
        self.model = MyPS3VLMModel(config)
